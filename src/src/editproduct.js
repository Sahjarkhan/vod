...
class Dashboard extends Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			], sperror: '', data: [], name: '', picturescolorchart: '', data4: [], data3: [], data1: [], pictures: [], pictures1: [], errors: {}
		};
		...
		//console.log();
		fetch("http://mobuloustech.com/yodapi/api/getproductdetails/" + params.userId).then((response) => response.json())
			.then((res) => {
				//alert(res);
				if (res.status === 'FAILURE') {
					...
					this.setState(function () {
						fetch("http://mobuloustech.com/yodapi/api/sublistbycatremark/" + res.response.subcategory_id).then((response) => response.json())
							.then((res) => {
								//alert(res);
								if (res.status === 'FAILURE') {
									toast.error(res.message);
					...
					this.setState({ multiValue: res.response.size1, multiValue1: res.response.color1 }, function () {
						fetch("http://mobuloustech.com/yodapi/api/sublistbycat/" + res.response.category_id).then((response) => response.json())
							.then((res) => {
								//alert(res);
								if (res.status === 'FAILURE') {
									toast.error(res.message);
	...
	handleSubmit(event) {
		...
		if (this.handleValidation()) {
			fetch("https://mobuloustech.com/yodapi/api/editproductbyseller/" + params.userId, {
				method: 'POST',
				headers: {
					Accept: 'application/json',
					'Content-Type': 'application/json',
													...
													<figcaption>
														<ul>
															{this.state.imagearray.map((item, key) =>
																<li><img alt="" src={item} /><span class="close">&times;</span></li>
															)}
